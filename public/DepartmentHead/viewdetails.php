    ...
    if (isset($_GET['internship'])) {
        ...
        if ($safe_slug !== null) {
            switch ($_SESSION['user_type']) {
                case 'DepartmentHead':
                    $results = $instance->getRequestsIdsAndSlugsForHod($_SESSION['id'], false);
                    break;

                case 'internship-supervisor':
                    $results = $instance->getIDwithSlug($_SESSION['id']);
                    break;
            }
...
if (!is_null($internshipNumber)) {
    $result = $instance->getRequestFullInformation($internshipNumber);
} else {
    header("Location: error404.php?what=empty_id");
    exit();
}
?>
