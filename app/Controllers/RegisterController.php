    ...
    try {
        foreach ($expected_input_names as $input_name) {
            if (!array_key_exists($input_name, $_POST)) {
                throw new Exception("Oops! Something went wrong with the form content. Please try again later.");
            }
        }

        foreach ($select_input_names as $select_name) {
        ...
        if (empty(trim($_POST['birth_date']))) {
            ...
            $birth_date = trim(filter_input(INPUT_POST, 'birth_date', FILTER_SANITIZE_FULL_SPECIAL_CHARS));
        ...
        if (empty(trim($_POST['student_card_id']))) {
            ...
            $student_card_id = trim(filter_input(INPUT_POST,'student_card_id', FILTER_SANITIZE_NUMBER_INT));
        ...
        if (empty($name_err) && empty($email_err) && empty($password_err) && empty($confirm_password_err) && empty($phone_err) && empty($address_err) && empty($department_err) && empty($grade_err) && empty($birth_date_err) && empty($birth_place_err) && empty($student_card_id_err)) {
            ...
            $is_verified = 0;
            $iddd=$student->createStudent($student_card_id, $department_id, $email, $username, $password_hash, $verification_code, $name, $birth_date, $birth_place, $grade, $phone, $is_verified);
            if (isset($iddd)) {
                $_SESSION["id"] = $iddd;
